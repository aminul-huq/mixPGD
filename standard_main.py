...
def main(learning_rate=5e-4, batch_size=20, epochs=10,
    ...
    model = nn.DataParallel(model, device_ids=[2,6])
    ...
    for epoch in range(0, epochs):        
        ...
        best_wer = test(model, device, test_loader, criterion, epoch, text_transform, filename, 'standard_new_')

    print("loading best model")    
    checkpoint = torch.load('./checkpoint/standard_new_model.pth.tar', map_location=device)
