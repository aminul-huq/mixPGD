...
def main(learning_rate=5e-4, batch_size=20, epochs=10,
    ...
    model = nn.DataParallel(model, device_ids=[7,8])
    ...
    filename = "FGSM_results"
    f = open(filename + ".txt", "w+")
    f.close()

    for epoch in range(0, epochs):        
        FGSM_train(model, device, train_loader, criterion, optimizer, scheduler,0.00004, text_transform, epoch)
        best_wer = test(model, device, test_loader, criterion, epoch, text_transform, filename, 'FGSM_new_')
        
    print("loading best model")    
    checkpoint = torch.load('./checkpoint/FGSM_new_model.pth.tar', map_location=device)
