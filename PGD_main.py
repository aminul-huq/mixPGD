...
def main(learning_rate=5e-4, batch_size=20, epochs=10,
    ...
    filename = "PGD_results" 
    f = open(filename + ".txt", "w+")
    f.close()

    for epoch in range(0, epochs):        
        PGD_train(model, device, train_loader, criterion, optimizer, scheduler,  0.00004,20,0.00001, text_transform, epoch)
        best_wer = test(model, device, test_loader, criterion, epoch, text_transform, filename, 'PGD_new_')

    print("loading best model")    
    checkpoint = torch.load('./checkpoint/PGD_new_model.pth.tar', map_location=device)
